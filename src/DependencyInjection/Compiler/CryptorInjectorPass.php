...
class CryptorInjectorPass implements CompilerPassInterface
{
     ...
     */
    public const ENCRYPTOR_AWARE_TAG = 'gtt.crypt.encryptor.aware';

    /**
     * Decryptor aware tag name
     */
    public const DECRYPTOR_AWARE_TAG = 'gtt.crypt.decryptor.aware';
    ...
    private static $tagConfig = [
        self::ENCRYPTOR_AWARE_TAG => [
            'target_interface'     => EncryptorAwareInterface::class,
            'setter'               => 'setEncryptor',
            'service_id_generator' => [CryptorServiceIdGenerator::class, 'generateEncryptorId'],
        ],
        self::DECRYPTOR_AWARE_TAG => [
            'target_interface'     => DecryptorAwareInterface::class,
            'setter'               => 'setDecryptor',
            'service_id_generator' => [CryptorServiceIdGenerator::class, 'generateDecryptorId'],
        ],
    ];
        ...
        foreach (self::$tagConfig as $tagName => $tagConfig) {
            foreach ($container->findTaggedServiceIds($tagName) as $id => $tags) {
                foreach ($tags as $tag) {
                    ...
                    if (!\in_array($tagConfig['target_interface'], class_implements($targetDefinitionClass), true)) {
                        throw new InvalidConsumerClassException($id, $targetDefinitionClass, $tagConfig['target_interface']);
                    }

                    $targetDefinition->addMethodCall($tagConfig['setter'], [new Reference($cryptorId), $name]);
                }
            }
        }
    }
